...
from RuleGeneration import Output as O
...
f = open('AllFiles/ReactionRules.txt','w')
db = open('DatabaseEntries.json','w')
sf = open('SmilesForReactions.txt','w')
...
for line in file:
    ...
    smilesfile = open('SmilesForReactions.txt','a')
...
#    print('rule R'+str(reactioncount)+'{', file=outputfile)
    for i in range(len(reactants)):
        ...
        mole, adj_mat, mol_formula = O.adsorbate.LoadByCovalentRadius(filepath, SurfaceAtomSymbols = SurfaceAtomSymbols)
#        if i <= 1:
...
#                print(atoms.GetProp('Label'))
        ...
        assert isinstance(mole,O.adsorbate)
#            outputfile = open('AllFiles/ReactionRules.txt','a')
...
#    print('\tconstraints{', file=outputfile)
    ...
    for i in range(len(reactant_molecules)):
        ...
        assert isinstance(adsorbate_molecules_reac[i],O.adsorbate)
    ...
    for i in range(len(product_molecules)):
        ...
        assert isinstance(adsorbate_molecules_prod[i],O.adsorbate)
